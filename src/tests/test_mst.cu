...
bool has_cycle(int v, int p, const std::vector<std::vector<int>> &g,
...
template <class T>
algos::MstAlgorithm *
create_mst_algo(); // implement parametrized function for each MST algorithm

template <> algos::MstAlgorithm *create_mst_algo<algos::PrimSpla>() {
  return new algos::PrimSpla();
}

template <> algos::MstAlgorithm *create_mst_algo<algos::BoruvkaSpla>() {
  return new algos::BoruvkaSpla();
}

template <> algos::MstAlgorithm *create_mst_algo<algos::BoruvkaGunrock>() {
  return new algos::BoruvkaGunrock();
}

template <> algos::MstAlgorithm *create_mst_algo<algos::PrimGunrock>() {
  return new algos::BoruvkaGunrock();
}

template <typename T> class MstAlgorithmTest : public ::testing::Test {
protected:
...
using AlgosTypes =
    ::testing::Types<algos::PrimSpla, algos::BoruvkaSpla, algos::BoruvkaGunrock,
                     algos::PrimGunrock>; // extend this with other MST
                                          // algorimths
TYPED_TEST_SUITE(MstAlgorithmTest, AlgosTypes);

static const GraphCase mst_test_cases[] = {{"point.mtx", 0},
                                           {"one_edge.mtx", 2},
                                           {"test1.mtx", 22},
                                           {"small.mtx", 120},
